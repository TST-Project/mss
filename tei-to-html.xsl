...
<my:scriptRef>
    ...
    <my:entry key="#tamilOE">long ō/ē (double-curled kōmpu)</my:entry>
...
<xsl:template match="x:g[@ref='#kompu']">
        <xsl:element name="span">
            <xsl:attribute name="class">gaiji aalt</xsl:attribute>
            <xsl:attribute name="data-anno">kōmpu</xsl:attribute>
            <xsl:text>&#x0B8E;</xsl:text>
        </xsl:element>
</xsl:template>
...
<xsl:template match="x:g[@ref='#nna=m']">
        <xsl:element name="span">
            ...
            <xsl:attribute name="data-anno">ṇam</xsl:attribute>
